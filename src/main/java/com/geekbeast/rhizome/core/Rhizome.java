...
public class Rhizome implements WebApplicationInitializer {
    ...
    private void registerDispatcherServlets( ServletContext servletContext ) {
        Map<String,DispatcherServletConfiguration> dispatcherServletsConfigs = rhizomeContext.getBeansOfType( DispatcherServletConfiguration.class , false , true );
        for( Entry<String, DispatcherServletConfiguration> configPair : dispatcherServletsConfigs.entrySet() ) {
            ...
            ServletRegistration.Dynamic dispatcher = servletContext.addServlet( configuration.getServletName() , new DispatcherServlet( dispatchServletContext ) );
    ...
    public static <T> T harvest( Class<T> clazz ) {
        return rhizomeContext.getBean( clazz );
    }
    
    public static void intercrop( Class<?> ... pods ) {
        rhizomeContext.register( pods );
    }
    
    public static void sprout() throws Exception {
    ...
    public static void wilt() throws BeansException, Exception {
        for( Loam loam : rhizomeContext.getBeansOfType( Loam.class ).values() ) {
            loam.stop();
        }
    }
     ...
     * To add additional configurations beyond the built in defaults, {@code intercrop(...)} 
